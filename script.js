...
const FACULTY_MAP = {
  ...
  'BCA': {
    ...
    title: 'Faculty – BCA Department',
    phone: '9547173980',             // ← update with real number
    image: 'bg_images/apu.jpg',      // ← your photo path
    dept:  'BCA'
  },

  'BBA': {
    name:  'Mr. Rahul Verma',
    title: 'HOD – BBA Department',
    phone: '919812345678',
    image: 'faculty-images/bba_faculty.jpg',
    dept:  'BBA'
  },

  'Hospital Management': {
    name:  'Dr. Anita Roy',
    title: 'HOD – Hospital Management',
    phone: '919800123456',
    image: 'faculty-images/hospital_faculty.jpg',
    dept:  'Hospital Management'
  },

  'Hotel Management': {
    name:  'Mr. Suresh Patel',
    title: 'HOD – Hotel Management',
    phone: '919898765432',
    image: 'faculty-images/hotel_faculty.jpg',
    dept:  'Hotel Management'
  },

  'BMLT': {
    name:  'Dr. Meena Das',
    title: 'HOD – BMLT Department',
    phone: '919845678901',
    image: 'faculty-images/bmlt_faculty.jpg',
    dept:  'BMLT'
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('waBackBtn').addEventListener('click', () => {
    hideModal('waModal');
    setTimeout(() => { if (activeDept) openFormModal(activeDept); }, 200);
  });
});
...
function openFormModal(dept) {
  ...
  iconEl.innerHTML = DEPT_SVG_ICONS[dept] || `<svg viewBox="0 0 48 48" fill="none" width="32" height="32"><circle cx="24" cy="24" r="18" stroke="currentColor" stroke-width="2.5"/></svg>`;
...
function initLiveValidation() {
  ['studentName', 'classId', 'semester', 'message'].forEach(id => {
    ...
    el.addEventListener('blur',  () => {
      const grp = el.closest('.form-group');
      el.value.trim() ? grp.classList.remove('has-error') : grp.classList.add('has-error');
    });
    el.addEventListener('input', () => {
      if (el.value.trim()) el.closest('.form-group').classList.remove('has-error');
    });
  });
}
...
function buildAndShowWa() {
  ...
  const faculty = FACULTY_MAP[dept];
  activeFaculty = faculty;

  // Build WhatsApp message text
  const waText =
`*CTMC Feedback Portal*
━━━━━━━━━━━━━━━━━━
...
_Sent via CTMC Feedback Portal_`;
  ...
  // Faculty photo — show image or SVG fallback
  ...
  // Switch modals
  closeFormModal();
  setTimeout(() => showModal('waModal'), 200);
}
...
function initModals() {
  ...
  document.getElementById('closeWaModal').addEventListener('click', () => hideModal('waModal'));

  ['formModal', 'waModal'].forEach(id => {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { hideModal('formModal'); hideModal('waModal'); }
  });
}
